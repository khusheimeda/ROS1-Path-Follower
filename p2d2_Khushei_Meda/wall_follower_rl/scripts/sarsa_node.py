...
# -*- coding: utf-8 -*-
"""
sarsa_node.py  --  SARSA wall follower
"""
...
# =====================================================================
#  STATE DISCRETISATION  (3 sectors × 3 bins = 27 states)
# =====================================================================
...
# =====================================================================
#  TRAINING SPAWNS  (same as Q-learning)
# =====================================================================
...
#  SARSA NODE
# =====================================================================
class SarsaNode(object):

    def __init__(self):
        rospy.init_node('sarsa_node', anonymous=False)

        self.mode        = rospy.get_param('~mode', 'train')
        default_qt_path  = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            'q_tables', 'q_table_sarsa.csv')
        ...
        # ---------- Q-table with expert initialization ----------
        ...
        if os.path.isfile(self.qt_path):
            if self.qtm.load_csv(self.qt_path):
                ...
                rospy.logwarn('Failed to load Q-table, using expert init')
                self._expert_init()
        else:
            rospy.loginfo('No existing Q-table, using expert initialization')
            self._expert_init()
    ...
    # -----------------------------------------------------------------
    #  Expert Q-value initialization (same as Q-learning)
    # -----------------------------------------------------------------
    def _expert_init(self):
        for s in range(NUM_STATES):
            right_bin       = s % 3
            front_right_bin = (s // 3) % 3
            front_bin       = (s // 9) % 3

            if right_bin == 1:
                self.qtm.set(s, 2, 5.0)
                self.qtm.set(s, 1, 2.0)
                self.qtm.set(s, 3, 2.0)
                if front_bin == 0:
                    self.qtm.set(s, 0, 8.0)
                    self.qtm.set(s, 1, 6.0)

            elif right_bin == 0:
                self.qtm.set(s, 0, 5.0)
                self.qtm.set(s, 1, 3.0)

            elif right_bin == 2:
                self.qtm.set(s, 4, 5.0)
                self.qtm.set(s, 3, 3.0)
                if front_bin == 2 and front_right_bin == 2:
                    self.qtm.set(s, 2, 4.0)

        rospy.loginfo('Expert Q-values initialized')
    ...
    def _choose_action(self, state, epsilon):
        if np.random.random() < epsilon:
            ...
            return self.qtm.best_action(state)
    ...
    # -----------------------------------------------------------------
    #  Reward  (identical to Q-learning)
    # -----------------------------------------------------------------
    @staticmethod
    ...
    # =================================================================
    #  TRAINING  —  THE SARSA DIFFERENCE FROM Q LEARNING
    # =================================================================
    def run_training(self):
        rospy.loginfo('=' * 60)
        rospy.loginfo('  SARSA TRAINING  ({} episodes, {} states)'.format(
        ...
        for ep in range(self.num_episodes):
            ...
            for step in range(MAX_STEPS_PER_EPISODE):
                ...
                # -- execute the already-chosen action --
                ...
                # *** SARSA: choose next action with epsilon-greedy ***
                next_action = self._choose_action(ns, epsilon)

                # *** SARSA TD update: uses Q(s', a') not max Q(s', a') ***
                old_q = self.qtm.get(state, action)
                td_target = r + self.gamma * self.qtm.get(ns, next_action)
                ...
                # *** SARSA: advance BOTH state AND action ***
                state = ns
                dists = nd
                action = next_action
    ...
    # =================================================================
    #  TESTING  (pure greedy, identical to Q-learning)
    # =================================================================
    def run_testing(self):
        ...
        rospy.loginfo('=' * 60)
        rospy.loginfo('  SARSA TESTING  (greedy policy)')
        rospy.loginfo('=' * 60)
